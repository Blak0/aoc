use std::fmt::Debug;
use std::fs::File;
use std::io::{prelude::*, BufReader};

use std::path::Path;

use itertools::Itertools;

#[derive(Default, Debug, Copy, Clone)]
struct WinDetails {
    turn_num: usize,
    last_number: u8,
}

impl WinDetails {
    fn new(last_number: u8, turn_num: usize) -> WinDetails {
        WinDetails {
            turn_num,
            last_number,
        }
    }
}

#[derive(Default)]
struct BingoTable {
    size: usize,
    win_details: Option<WinDetails>,
    number_table: Vec<Vec<u8>>,
    checked_table: Vec<Vec<bool>>,
}

impl Debug for BingoTable {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BingoTable")
            .field("size", &self.size)
            .field("win_details", &self.win_details)
            .finish()
    }
}

impl BingoTable {
    fn new(id: u8, table: Vec<Vec<u8>>) -> BingoTable {
        BingoTable {
            size: table
                .get(0)
                .unwrap_or_else(|| panic!("Error when parsing {id} table"))
                .len(),
            win_details: None,
            number_table: table,
            checked_table: vec![vec![false; 5]; 5],
        }
    }

    fn make_a_move(&mut self, m: u8, turn_num: usize) {
        if self.is_won() {
            return;
        }

        for (row_idx, row) in self.number_table.iter().enumerate() {
            for (col_idx, number) in row.iter().enumerate() {
                if m == *number {
                    self.checked_table[row_idx][col_idx] = true;
                }
            }
        }

        self.win_check(m, turn_num);
    }

    fn win_check(&mut self, m: u8, turn_num: usize) {
        for i in 0..self.size {
            if self.checked_table[i][0..self.size].iter().all(|x| *x)
                || (0..self.size)
                    .map(|idx| self.checked_table[idx][i])
                    .all(|cell| cell)
            {
                self.win_details = Some(WinDetails::new(m, turn_num));
            }
        }
    }

    fn is_won(&self) -> bool {
        self.win_details.is_some()
    }

    fn calculate_score(&self) -> Option<u32> {
        if !self.is_won() {
            return None;
        }
        let mut sum = 0u32;
        for row_idx in 0..self.size {
            for cell_idx in 0..self.size {
                if !self.checked_table[row_idx][cell_idx] {
                    sum += self.number_table[row_idx][cell_idx] as u32;
                }
            }
        }
        let score = self.win_details.unwrap().last_number as u32 * sum;
        Some(score)
    }
}

pub fn solution() {
    let path = Path::new("files/4.txt").canonicalize().unwrap();

    let f = File::open(path).expect("file not found");

    let reader = BufReader::new(f);

    let mut lines = reader.lines().filter_map(|x| x.ok());

    let moves = lines
        .next()
        .expect("File empty")
        .split(',')
        .map(str::parse::<u8>)
        .map(Result::unwrap)
        .collect_vec();

    let tables = lines
        .chunks(6)
        .into_iter()
        .map(|chunk| {
            chunk
                .skip(1)
                .take(5)
                .map(|line| {
                    line.split(' ')
                        .filter(|x| !x.is_empty())
                        .map(str::to_owned)
                        .collect_vec()
                })
                .map(|x| {
                    x.into_iter()
                        .map(|x| x.parse::<u8>())
                        .map(Result::unwrap)
                        .collect_vec()
                })
                .collect_vec()
        })
        .collect_vec();

    let mut tables = tables
        .into_iter()
        .enumerate()
        .filter(|(_, table)| !table.is_empty())
        .map(move |(idx, table)| BingoTable::new(idx as u8, table))
        .collect_vec();

    moves.iter().enumerate().for_each(|(turn_num, m)| {
        tables
            .iter_mut()
            .for_each(|table| table.make_a_move(*m, turn_num))
    });

    let winners = tables
        .into_iter()
        .filter(BingoTable::is_won)
        .sorted_by(|tab1, tab2| {
            tab1.win_details
                .unwrap()
                .turn_num
                .cmp(&tab2.win_details.unwrap().turn_num)
        })
        .collect_vec();

    let last_winner_score = winners
        .iter()
        .rev()
        .next()
        .unwrap()
        .calculate_score()
        .unwrap();
    println!("{last_winner_score}")
}
